...
double CalculSommeTauAnnot(double **Pi,double **Tau,double **G, int *annot, int debut, int fin,int j,int t)  
{
...
/******************************************************************************/
  ...
  double sumFPi2Annot(double **Pi,double **F, int *annot, int debut, int fin,int j,int t)  
{
   //Déclaration et initialisation des variables locales;      
   int x;
   double res;
...
void CalculGAnnot(double **Pi,double **F, int *annot, int nbClass,int nbInd,double **G)  
{
     //Déclaration et initialisation des variables locales;   
     int x,y;
     ...
     for(x=0; x<(nbInd-1); x++){
                     for(y=0; y<nbClass; y++){                                      
                             G[x+1][y] =sumFPi2Annot(Pi,F,annot,0,nbClass,y,x);   
			     G[0][y] = 0;                                
                    }         
     }   
}
...
void CalculTauAnnot(double **Pi,double **F, int *annot, int nbClass,int nbInd,double **G,double **Tau)  
{
     //Déclaration et initialisation des variables locales;   
     int x,y;
     double NormF = 0;
...
void CalculTauNAnnot(double **F,int nbInd, int nbClass, double **Tau)  
{
    int y;
    for(y=0; y<nbClass; y++){ 
...
void Backward_Annot(double *FVect, double *PiVect,int *annot, int *nbInd, int *nbClass, int *p, double *TauVect, double *GVect)  
{
     int i;
  ...
  double **G = (double**)malloc(*nbInd*sizeof(double*));
    ...
    CalculTauNAnnot(F,*nbInd,*nbClass,Tau); 
       
    //Calcul des autres termes(de l'instant 1 à t-1);           
    CalculTauAnnot(Pi,F,annot,*nbClass,*nbInd,G,Tau);
