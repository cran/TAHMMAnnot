...
resF = ForwardAnnotrestart0R(Phi.vect,muHMM.vect,Mat.trans.norm.vect,annot,diffannot,n,K,p)

resB = BackwardAnnotrestart0R(muHMM.vect,resF$F,Mat.trans.norm.vect,annot,diffannot,n,K,p)
...
for (an in 1:p)
{
   tibis = ti[index.annot[[an]],]
   Fbis = F[index.annot[[an]]-1,]
   Gbis = G[index.annot[[an]],]
   if(index.annot[[an]][1]==1)
   {
    tibis = tibis[-1,]
    Gbis = Gbis[-1,]
   }
